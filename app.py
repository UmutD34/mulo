...
.block-container { padding: 1.4rem 1rem 4rem !important; max-width: 720px !important; }

.hud-wrap {
    display: flex; align-items: center; justify-content: space-between;
    background: #13171f; border: 1px solid #252b36; border-radius: 14px;
    padding: 14px 20px; margin-bottom: 18px; position: relative; overflow: hidden;
}
.hud-wrap::after {
...
.hud-title { font-family:'Rajdhani',sans-serif; font-size:22px; font-weight:700; color:#4da3ff; letter-spacing:3px; text-transform:uppercase; }
.hud-sub { font-family:'Share Tech Mono',monospace; font-size:10px; color:#4a5568; letter-spacing:2px; margin-top:3px; }
.hud-pill { background:#0b0d11; border:1px solid #252b36; border-radius:8px; padding:6px 16px; text-align:center; }
...
.stat-box { flex:1; background:#13171f; border:1px solid #252b36; border-radius:10px; padding:12px; text-align:center; }
...
.prog-meta { display:flex; justify-content:space-between; font-family:'Share Tech Mono',monospace; font-size:10px; color:#6b7280; margin-bottom:6px; }
...
.q-tag { font-family:'Share Tech Mono',monospace; font-size:10px; color:#4a5568; letter-spacing:3px; text-transform:uppercase; margin-bottom:12px; display:flex; align-items:center; gap:8px; }
.q-id { background:#0b0d11; border:1px solid #252b36; border-radius:4px; padding:2px 8px; font-size:10px; color:#4da3ff; }
.q-text { font-family:'Noto Sans',sans-serif; font-size:18px; font-weight:700; color:#f0f4f8; line-height:1.55; }
.q-foot { margin-top:14px; font-family:'Share Tech Mono',monospace; font-size:9px; color:#4a5568; letter-spacing:2px; }
...
.a-head { font-family:'Share Tech Mono',monospace; font-size:10px; color:#3fb950; letter-spacing:3px; margin-bottom:12px; }
...
div[data-testid="column"]:nth-child(2) .stButton > button:hover:not(:disabled) { background:#0e1a13 !important; border-color:#3fb950 !important; color:#56d364 !important; }

div[data-testid="column"]:nth-child(4) .stButton > button { border-color:#7a3f10 !important; color:#f0883e !important; }
div[data-testid="column"]:nth-child(4) .stButton > button:hover:not(:disabled) { background:#1a1208 !important; color:#ffa657 !important; }
...
details > summary { background:#13171f !important; border:1px solid #252b36 !important; border-radius:8px !important; color:#6b7280 !important; font-family:'Share Tech Mono',monospace !important; font-size:11px !important; letter-spacing:2px !important; padding: 12px 16px !important; }
...
with st.expander("⚙️  FİLTRELER & AYARLAR"):
    ...
    mode_icon = "📋 SIRALI" if st.session_state.mode == "sıralı" else "🔀 RASTGELE"
    st.markdown(
        f"<div style='font-family:Share Tech Mono,monospace;font-size:10px;"
        f"color:#4a5568;letter-spacing:2px;padding-top:6px'>"
        f"AKTİF MOD · <span style='color:#4da3ff'>{mode_icon}</span></div>",
        unsafe_allow_html=True
    )
...
<div class="a-card">
  ...
  <div class="a-body">{cevap}</div>
    ...
    # Google Arama Butonu
    arama_sorgusu = urllib.parse.quote_plus(q["soru"])
    google_url    = f"https://www.google.com/search?q={arama_sorgusu}"

    st.markdown(
        f'<a href="{google_url}" target="_blank" style="'
        'display:block; text-align:center; margin-bottom:12px;'
        'background:#0b0d11; border:1px solid #30363d; border-radius:8px;'
        'padding:11px 0; font-family:Rajdhani,sans-serif; font-weight:700;'
